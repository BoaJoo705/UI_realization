...
public class BoardDao {
   ...
   public int insert(Board board) throws Exception {
	...
	public Board selectOne(int boardNo) throws Exception {
		...
		final String sqlSelectOne = "SELECT RBOARD_NO,WRITER,RBOARD_TITLE,RBOARD_CONTENT,REG_DATE FROM REVIEWBOARD" + " WHERE RBOARD_NO=";
		...
		try {
			...
			if (rs.next()) {
				board = new Board().setBoardNo(rs.getInt("RBOARD_NO"))
									.setWriter(rs.getString("WRITER"))
									.setBoardTitle(rs.getString("RBOARD_TITLE"))
									.setBoardContent(rs.getString("RBOARD_CONTENT"))
									.setRegistrationDate(rs.getDate("REG_DATE"));
				...
				throw new Exception("해당 번호의 글 찾을 수 없습니다.");
			}
	...
	public int update(Board board) throws Exception {
		...
		int result = 0;
		PreparedStatement stmt = null;
		final String sqlUpdate = "UPDATE REVIEWBOARD SET RBOARD_TITLE=?,RBOARD_CONTENT=?,REG_DATE=now()" + " WHERE RBOARD_NO=?";
		try {
			// 커넥션 풀에서 Connection객체를 빌려온다
			connection = ds.getConnection();
